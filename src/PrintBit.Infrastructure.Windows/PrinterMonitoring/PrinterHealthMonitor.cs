...
public class PrinterHealthMonitor : BackgroundService, IPrinterHealthMonitor
{
    ...
    private static PrGetStatusCodeDelegate? _prGetStatusCode;
    private static readonly object EpsonApiInitializationLock = new();
    private static bool _epsonApiInitialized;
    ...
    private static void EnsureEpsonApiInitialized()
    {
        ...
        lock (EpsonApiInitializationLock)
        {
            ...
            {
                // The standard WinSpool and WMI sources remain available when
    ...
    public virtual bool IsHealthy(string printerName, out int winSpoolStatus, out string winSpoolDesc)
    {
        ...
        var epsonHealthy = !TryGetEpsonDriverStatusCode(
            printerName,
            out var epsonStatusCode,
            out _) || !IsFatalDetectedErrorState(epsonStatusCode);
    ...
    protected virtual async Task MonitorPrinterAsync(CancellationToken stoppingToken)
    {
        ...
        if (foundPrinter)
        {
            ...
            var hasEpsonStatus = TryGetEpsonDriverStatusCode(
                _hardwareSettings.PrinterName,
                out var epsonStatusCode,
                out var epsonStatusDescription);
            var isFatalEpsonStatus =
                hasEpsonStatus && IsFatalDetectedErrorState(epsonStatusCode);
