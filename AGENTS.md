...
- `PrinterHealthMonitor` checks the Epson driver status code (`PrGetStatusCode`), WMI `DetectedErrorState`, `ExtendedPrinterStatus`, and WinSpool status, treating fatal codes (e.g. Paper Out code 4, Jam code 8, Door Open code 7) as fatal errors exposed via `IsHealthy` and `HasFatalHardwareError`.
...
- `JobOrchestrator` emits `PrintSucceeded` only after every requested copy completes. Partial or terminal failures emit `PrintFailed` with page/copy results and move the source PDF and sidecar to `failed/`.
...
| `pagesPrinted` | No | Completed logical pages across copies; best effort on failure |
| `totalPages` | No | Total selected pages across copies on terminal events |
| `pageCountConfidence` | No | `confirmed`, `best_effort`, or `unknown` |
| `pages` | No | Per-page/copy terminal states (`completed`, `failed`, or `cancelled`) |
...
- Remove or bypass `SemaphoreSlim(1, 1)` in `DocumentPrinter`.
...
- If the spooler job clears and the printer is completely healthy (no error reported via Epson driver `PrGetStatusCode`, WMI, Epson popups, or direct WinSpool checks), the print is treated as a success regardless of WMI page count reporting lag, provided the last polled spooler `TotalPages` is not less than the expected page count (a lower `TotalPages` indicates a truncated/aborted job).
...
- `PrinterHealthMonitor` reports fatal driver/WMI error states (such as Paper Out) to Node.js as `PrinterError` and exposes them to in-flight print verification.
...
| Print dispatch granularity or page-count confidence changed | Sections 2, 3, and 6 |
