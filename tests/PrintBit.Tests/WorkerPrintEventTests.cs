...
public class WorkerPrintEventTests
{
    ...
    [Fact]
    public void WorkerPrintEvent_SerializesAdditiveV2Fields()
    {
        var evt = new WorkerPrintEvent
        {
            Type = WorkerPrintEventType.JobCompleted,
        ...
        Assert.Equal("event-001", document.RootElement.GetProperty("eventId").GetString());
    ...
    [Fact]
    public void WorkerPrintEvent_LegacyFrameOmitsUnsetV2FieldsAndPreservesPauseReason()
    {
        var evt = new WorkerPrintEvent
        {
        ...
        using var document = JsonDocument.Parse(json);
        Assert.False(document.RootElement.TryGetProperty("protocolVersion", out _));
        ...
        Assert.Equal("JobPaused", document.RootElement.GetProperty("type").GetString());
        Assert.Equal("tx-1", document.RootElement.GetProperty("transactionId").GetString());
        Assert.Equal("spool-1", document.RootElement.GetProperty("spoolerCorrelationKey").GetString());
        Assert.Equal("hardware", document.RootElement.GetProperty("pauseReason").GetString());
        Assert.Equal("Paper out", document.RootElement.GetProperty("message").GetString());
    }

    [Fact]
    public void IpcSettings_DefaultsWorkerCommandPipeName()
    {
        var settings = new IpcSettings();

        Assert.Equal("printbit-worker-commands", settings.WorkerCommandPipeName);
    }
}
