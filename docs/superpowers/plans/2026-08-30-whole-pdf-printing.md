...
- Preserve paper-out/offline pause and resume behavior for the same spooler job without emitting Node-unsupported intermediate events.
...
- Create: `src/PrintBit.Infrastructure/Services/PrintService/DocumentPrintResult.cs`
- Create: `tests/PrintBit.Tests/DocumentPrinterTests.cs`

**Interfaces:**
- Consumes: `IPrinterHealthMonitor`, `HardwareSettings`, and `PrintJobSettings`.
- Produces: `IDocumentPrinter.PrintDocumentAsync` plus `DocumentPrintResult.PagesPrinted`, `TotalPages`, and `PageCountConfidence`.

- [x] **Step 1: Write failing tests for Sumatra settings and spooler completion**
...
- [x] **Step 5: Commit `feat: dispatch whole PDF copies`**
...
- Produces: one `PrintStarted` and exactly one `PrintSucceeded` or `PrintFailed` terminal event with page/copy results and count confidence; intermediate progress remains internal.
...
- [x] **Step 3: Implement copy-level orchestration**
...
- [x] **Step 5: Commit `feat: orchestrate whole PDF copies`**
...
- [x] **Step 1: Write and verify the failing configuration expectation**
...
- [x] **Step 2: Remove split-only configuration and synchronize docs**
...
- [x] **Step 4: Commit `docs: document whole PDF printing architecture`**
